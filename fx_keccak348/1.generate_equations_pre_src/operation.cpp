...
//copy equation
void copy_equs(char* state_des, char* state_src){
    
    int n = 0;
    while (state_src[n] != ':') {
        state_des[n] = state_src[n];
        n++;
    }
    state_des[n] = ':';
}
...
//initial variables
void initVars_sym(char state[Five][Five][LANE][equLength], bool vars[Five * Five], int i){
            ...
            if (vars[x+ Five * y]) {
                for (int z = 0; z < LANE; z++) {
                    if(z < i){
                        ...
                        copy_equs(state[x][y][z], state[x][y][z%i]);
                    }
                }
            }
        }
